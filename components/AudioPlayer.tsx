    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.code) {
        ...
        case 'ArrowLeft':
          ...
          skip(-10);
          break;
        // 已移除 ArrowUp/Down，改由 TranscriptViewer 處理
      }
    };
    ...
    <div className="w-full h-full flex flex-col justify-center px-4">
      ...
      <div className="flex items-center space-x-6">
        <div className="flex items-center space-x-2 shrink-0">
          ...
          <button 
            onClick={() => skip(-10)}
            className="p-2 text-slate-400 dark:text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
            title="後退 10 秒"
          >
        ...
        <div className="flex-1 flex flex-col space-y-1">
          <div className="flex justify-between items-center px-1">
            <span className="text-[11px] font-bold font-mono text-indigo-600 dark:text-indigo-400">{formatTime(currentTime)}</span>
            <span className="text-[11px] font-bold font-mono text-slate-300 dark:text-gray-600">{formatTime(localAudioRef.current?.duration || 0)}</span>
          </div>
          <div 
        ...
        <div className="hidden sm:flex items-center space-x-3 shrink-0">
          <svg className="w-5 h-5 text-slate-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input 
            ...
            className="w-24 h-1 bg-slate-100 dark:bg-gray-800 rounded-lg appearance-none cursor-pointer accent-indigo-500"
          />
        </div>
      </div>
    </div>
